...
def send_notificacion(user_id, mensaje):
    ...
    notificacion = Notificacion.objects.create(mensaje=mensaje, user=user)
    return notificacion

def get_notificaciones(user_id):
    user = get_object_or_404(User, id=user_id)
    notificaciones = Notificacion.objects.filter(user=user)
    return notificaciones

def count_unread_notificaciones(user_id):
    user = get_object_or_404(User, id=user_id)
    notificaciones = Notificacion.objects.filter(user=user, read = False)
